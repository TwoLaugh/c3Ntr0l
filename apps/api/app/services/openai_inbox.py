...
def _instructions() -> str:
    return (
        ...
        "Use propose_plan_change when the user asks to change the current day or when inserting work today "
        "would disrupt existing timing. The first supported proposal is insert_item_today. "
        "For propose_plan_change, include proposed_change_type and proposed_change_payload. "
        "The payload for insert_item_today should include item_id when an existing item is involved; "
        "if the item does not exist, create_item first and then propose only when the user explicitly wants it scheduled today. "
